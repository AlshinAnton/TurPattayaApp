...
public class Utility {
    ...
    private static final String regexpRuble ="Russian Ruble\\s*</td><td class=\"D3\">(\\d+\\.\\d+)" +
            "\\s*</td><td class=\"D4\">\\W</td><td class=\"D4\">\\W</td><td class=\"D4\">\\W" +
            "</td><td class=\"D4\">\\W</td><td class=\"D5\">(\\d+\\.\\d+)\\s*</td></tr>";
...
/*    private static final Pattern patternUSD1 = Pattern.compile(regexpUSD1);
    ...
    public static String getRuble(String text) {
        String result = "";
        Matcher matcherRuble = patternRuble.matcher(text);

        if (matcherRuble.find()) {
            result = matcherRuble.group(1);
        }
        return result;
    }
}
